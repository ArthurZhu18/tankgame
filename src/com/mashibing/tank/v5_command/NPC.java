...
public class NPC extends AbstractGameObject {
    ...
    private void boundsCheck() {
        if (x<0 || y<30 || x + wight> TankFrameSingleton.GAME_WIDTH
                || y + height> TankFrameSingleton.GAME_HEIGHT){
            this.back();
        }
    }
    ...
    public void fire() {
        ...
        TankFrameSingleton.INSTANCE.getGm().add(new Bullet(bX, bY, dir, group));
    }

    public void die() {
        live = false;
        TankFrameSingleton.INSTANCE.getGm().add( new Explode(x ,y));
    }
